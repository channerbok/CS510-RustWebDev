        ...
        Ok(account) => match verify_password(&account.password, login.password.as_bytes()) {
            Ok(verified) => {
                if verified {
                    ...
                    let json_response =
                        serde_json::to_string(&token).map_err(|_e| MyError::SerializationError)?;
                    Ok((StatusCode::OK, json_response).into_response())
                } else {
                    Err(MyError::WrongPassword)
                }
            }
            Err(e) => Err(MyError::ArgonLibraryError(e)),
        },
        Err(e) => Err(e),
    }
}
...
pub fn verify_token(token: String) -> Result<Session, MyError> {
    ...
    .map_err(|_| MyError::WrongPassword)?;
    serde_json::from_value::<Session>(token).map_err(|_| MyError::WrongPassword)
}
